...
const CryptoItem = ({crypto}) => {
    ...
    return (<Link to={`/crypto/${crypto.CoinInfo.Name}`} underlayColor={"#d5d5d5"}
                  className={"my-2 mx-4 p-3 bg-white rounded-lg shadow-sm shadow-black"}>
        <View className={"flex flex-row items-center justify-between"}>
            <View className={"flex flex-row items-center"}>
                <Image source={{uri: `https://cryptocompare.com/${crypto.CoinInfo.ImageUrl}`}}
            ...
            <View className={"flex flex-col items-start justify-between w-24"}>
                ...
                <Text
                    className={`${crypto.RAW.USD.CHANGEPCT24HOUR < 0 ? "text-red-500" : "text-green-500"}`}>
                    {crypto.RAW.USD.CHANGEPCT24HOUR.toFixed(2)}%
                </Text>
            </View>
        </View>
    </Link>);
